...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
			 ...
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                welcomeBut = [{buttonId:`#owner`,buttonText:{displayText:'OWNER'},type:1}, {buttonId:`mew`,buttonText:{displayText:'WELCOME'},type:1}]
            ...
            if (anu.action == 'remove' && !mem.includes(client.user.jid)) {
                ...
                goodbyeBut = [{buttonId:`#h`,buttonText:{displayText:'Byee👋'}, type:1}]
...
if (!isUser) return reply(mess.daftar)
			if (isBanned) return reply(mess.banned)
					...
					if (!isUser) return reply(mess.daftar)
if (isBanned) return reply (mess.banned)
buff = await getBuffer (`https://kuontol-api.herokuapp.com/api/textpro/impressive-glitch-effect?text=${args.join(' ')}`)
...
case "closegc": 
          ...
          if (isBanned) return reply (mess.banned)
           ...
           if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://hardianto-chan.herokuapp.com/api/bot/gura?apikey=${hardikey}&nama=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
           case 'candytext':
           if (isBanned) return reply (mess.banned)
           ...
           if (isBanned) return reply(mess.banned)
            ...
            if (isBanned) return reply(mess.banned)
             ...
             if (isBanned) return reply(mess.banned)
					...
					if (args.length < 1) return reply(`Example: ${prefix + command}Bot&Indo`)
					reply(mess.wait)
					buffer = await getBuffer(`https://melcanz.net/girlneko?text=${gli}&text2=${tch}&apikey=daff`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: mess.success})
					break
             case 'sadlogo':
             if (isBanned) return reply(mess.banned)
					...
					if (args.length < 1) return reply(`Example: ${prefix + command}Bot&Indo`)
					reply(mess.wait)
					buffer = await getBuffer(`https://melcanz.net/sadboy?text=${gli}&text2=${tch}&apikey=daff`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: mess.success})
					break
            case 'lighttext':
            if (isBanned) return reply (mess.banned)
   ...
   if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://melcanz.net/kaneki?text=${txt}&apikey=daff`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
           case 'lavatext':
           if (isBanned) return reply (mess.banned)
            ...
            if (isBanned) return reply (mess.banned)
              ...
              if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://melcanz.net/lolimaker?text=${txt}&apikey=daff`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
              case 'logoepep':
              if (isBanned) return reply (mess.banned)
   ...
   sendButImage(from, pll, `Klik Next Untuk Melanjutkan`, kon, [                      
          {
            ...
            type: 1,
                ...
                if (isBanned) return reply (mess.banned)
                reply(mess.wait)
                buffer = await getBuffer (`https://api-velgrynd.herokuapp.com/api/randomimage/loli`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: command})
                break
                case 'shinobu':
                if (isBanned) return reply(mess.banned)
                reply(mess.wait)
            ...
            case 'sagiri':
				...
				if (isBanned) return reply(mess.banned)
					...
					if (args.length < 1) return reply(`Example: ${prefix + command} Bot&Indo`)
					reply(mess.wait)
					buffer = await getBuffer(`https://api.dapuhy.ga/api/textpro/pornhub?text1=${gli}&text2=${tch}&apikey=wf2tghNhfU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: mess.success})
					break
                case 'ytsearch':
...
tod = await getBuffer(`https://telegra.ph/file/93fd4cb214d27997aec65.jpg`)
                 ...
                 case 'script':
                case 'sc':
                reply(`*BOT INI MENGGUNAKAN SC*\n*https://github.com/Toni33812/botprii* \n*Jangan lupa star + follow*`)
                break
   case 'bokep':
